...
(function ($) {
    ...
    $.fn.date = function (options,Ycallback,Ncallback) {   
        ...
        $.fn.date.defaultOptions = {
            ...
            beginday:1,                     //日期--日--份结束
            endday:31,                      //日期--日--份结束
            beginhour:options.checkHStart || 1,
            endhour:options.checkEnd || 24,
            beginminute:   00,
            endminute:     59,
			  ...
			  $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length-1);
               if(datetime){
                ...
                if(Ycallback===undefined){
                     if(docType){
                    	 if(that.hasClass("changePreTime") ){
                    		 that.val("修改预约时间");
                    	 }else{
                    		 that.val(datestr);
                    	 }
                    	 that.parent("p").parent("div").parent("div").prev("div").children("p").first().text("预约时间："+datestr);
                    	 that.attr("data-changed-time",datestr);
                    	 that.closest("ul").find(".reservTime").text(datestr);
                         that.parent("p").parent("div").prev("ul").children("a").children("li").children("p[data-order-id="+options.orderId+"]").text("预约时间："+datestr);
                     }else{
                    	 if(that.hasClass("changePreTime") ){
                    		 that.html("修改预约时间");
       ...
       //david
        ...
        function CreateDateUI(){
            ...
            if(opts.orderId){
            	$("div[data-shop-id="+ opts.orderId+"]").html(str);
            }else{
            	 $("#datePlugin").html(str);
            }
    	...
    	//david 对比店铺的营业时间  时 ：分
    	var checkTime = function(changedTim){
    		var changeTime = changedTim.split(" ")[1];
    		var hours = changeTime.split(":")[0];
    		var times = changeTime.split(":")[1];
    		console.log("hours",hours,"times",times);
    		
    		var checkHStart = (shopInfoObj.shop_bussiness_time).toString().split("-")[0];
    	     checkHStart = parseInt(checkHStart.split(":")[0]);
    	     
    	   var checkEnd = (shopInfoObj.shop_bussiness_time).toString().split("-")[1];
    	     checkEnd = parseInt(checkEnd.split(":")[0]);
    		
    	    if(hours<checkHStart || hours > checkEnd){
    	    	
    	    	layer.open({
    	    		content:"超出店铺的营业时间"+shopInfoObj.shop_bussiness_time,
    	    		type:1,
    	     ...
    	     console.log("checkHStart",checkHStart,"checkEnd",checkEnd);
